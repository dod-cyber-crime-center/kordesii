...
def multiproc_set_level(level):
    """Multiproc initializer used to pass along log level."""
...
class Tester(object):
    ...
    def parse_results_file(self, results_file_path):
        ...
        assert isinstance(data, list) and all(isinstance(a, dict) for a in data)
    ...
    def run_tests(
        ...
        pool = mp.Pool(
            processes=procs, initializer=multiproc_set_level, initargs=(logging.root.getEffectiveLevel(),))
    ...
    def compare_results_field(self, results_a, results_b, field_name):
        ...
        if field_name == kordesii.reporter.FIELD_STRINGS:
            comparer = StringsComparison()
        elif field_name == kordesii.reporter.FIELD_FILES:
...
class StringsComparison(ResultComparison):
    def __init__(self):
        super(StringsComparison, self).__init__(kordesii.reporter.FIELD_STRINGS)


class FilesComparison(ResultComparison):
    def __init__(self):
        super(FilesComparison, self).__init__(kordesii.reporter.FIELD_FILES)
